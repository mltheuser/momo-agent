...
/**
 * Whether the log's trailing state is an unanswered
 * [AgentEvent.QuestionAsked] — while parked no other input is possible, so
 * the last pause-or-run event decides.
 */
private fun endsParked(events: List<AgentEvent>): Boolean =
    events.lastOrNull { it.decidesParking } is AgentEvent.QuestionAsked
