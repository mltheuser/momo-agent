...
3. **Retry** after a run ended `error` (the router was down). The failed LLM call and its `run_finished` are cut from the log and the session is running again at once, opened by `run_resumed` instead of a new `run_started`: the model is asked the same question it failed to answer, with the same history, under the run's recorded model and reasoning effort. Progress before the failure survives.
