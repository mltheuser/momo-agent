...
`idle` otherwise. It comes from the server's memory, never from the log: a log
left open by a kill is not a running session.

A run in flight has no `run_finished` yet; every graceful end of a run leaves
the log settled. Killing the server mid-run is the one way to leave a log
unsettled after the fact. Before serving any read of a session that is not
running, the server settles it on disk, so a reader never sees an open run
that nobody is executing.
...
3. **Retry** after a run ended `error` (the router was down). The log is cut back to the last message before the failed LLM call — the call itself, its `run_finished` and anything between them go — and the agent is loaded from what remains and asked again under the run's recorded model and reasoning effort. Because a chat completion sends the whole history, the same history is the same call. Nothing records that a failure happened: after a successful retry the log reads as if the call had succeeded the first time, and a second or third retry of a call that keeps failing leaves the log in the same shape each time. Progress before the failure survives.
